...
String upUrl = "http://bin.bemfa.com/b/3Bc**************************WM=DemoLight002.bin";
String Meskey = "SCT***********************************U7Er";
//**************************************************//
// 最大字节数
...
void SendMes(String title)
{
  HTTPClient http;
  http.begin(TCPclient,"http://sctapi.ftqq.com/" + Meskey + ".send?title=" + title);
...
void servodriver(int deg) // 定义一个脉冲函数
{
  int myangle1 = map(deg, 0, 180, 500, 2500);
  digitalWrite(D2, HIGH);      // 将舵机接口电平至高
  delayMicroseconds(myangle1); // 延时高电平的微秒数
  digitalWrite(D2, LOW);       // 将舵机接口电平至低
  delayMicroseconds(20000 - myangle1); // 延时低电平的微秒数
}
